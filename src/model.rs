...
impl Model {
    ...
    pub fn load_model(
        ...
        for m in obj_materials? {
            ...
            let diffuse_texture = Self::load_texture(&format!("./res/{diff}"), device, queue,false).unwrap();
            let norm = &m.normal_texture.unwrap();

            let normal_texture = Self::load_texture(&format!("./res/{norm}"), device, queue,true).unwrap();
